...
**`C:\Parti` 에서 Claude Code 를 연다.** 끝이다.

저장소에 `.mcp.json` 이 있어서 `parti` 서버를 쓸지 한 번 물어본다. 승인하면
서버가 뜨고 → **브라우저가 알아서 열리고** → 연결된다. 상태바의 **MCP** 칩이 초록이면 된 것이다.
이제 Claude 에게 그냥 말하면 된다 — *"3층짜리 두 동 세우고 도면세트 뽑아 줘"*.

이미 Parti 탭을 켜 두고 있었다면 새 창을 열지 않고 그 탭이 알아서 붙는다.
자동 열기가 싫으면 `--no-open` (또는 `PARTI_MCP_OPEN=0`).

> **Claude Code 를 이미 열어 둔 상태였다면 한 번 껐다 켜야 한다** — MCP 설정은 시작할 때만 읽는다.
> 이것 하나가 "왜 로컬 모드지?" 의 대부분이다.

다른 폴더에서 쓰거나 수동으로 붙이려면:

```bash
claude mcp add parti -- node C:/Parti/parti-mcp/server.js
```

Claude 없이 도면만 보려면 서버만 띄워도 된다 — `node parti-mcp/server.js`.

## 안 될 때 — 코워커 패널의 MCP 단추

Parti 코워커 패널 제목줄의 **MCP** 단추(또는 하단 상태바의 MCP 칩)를 누르면 세 조각을 각각 짚어 준다.

| | 안 되면 |
|---|---|
| ① 서버 | 실행 명령을 복사해 준다 |
| ② 브라우저 | 브리지 주소로 여는 단추를 준다 |
| ③ Claude | 등록 명령을 복사해 준다 (작업본 경로를 서버에서 받아 조립) |

창을 열어 둔 채 서버를 켜면 3초 안에 초록으로 바뀐다.

### 단추 색이 곧 상태다 — 셋을 구분한다

| 색 | 뜻 | 코워커 인사말 |
|---|---|---|
| 회색 | 브리지도 안 붙음 | "지금은 로컬 모드입니다" |
| **노랑** | 브리지는 붙었지만 **Claude 가 안 붙음** | "Claude Code 를 열고 parti 를 승인하세요" |
| 초록 | Claude 까지 붙음 | "여기에 그냥 말하면 됩니다" |

★노랑을 초록과 뭉뚱그리면 안 된다. 서버가 떠서 브라우저는 붙었는데 Claude 는 없는 상태가
사용자가 가장 헷갈리는 지점이고, 그때 "Claude 에게 말하세요" 라고 안내하면 거짓말이 된다.
서버가 SSE 로 `{state:{claude}}` 를 보내 이 셋을 브라우저가 구분한다(붙는 즉시 · Claude 가
나중에 붙어도 다시).

## 코워커 입력창이 곧 Claude 다

연결(초록)되면 Parti 코워커 패널의 입력창이 **로컬 파서에서 Claude 로 바뀐다.** 거기에
*"3층짜리 두 동 세우고 도면세트 뽑아줘"* 라고 쓰면 화면의 도면이 바뀐다. Claude Code 창으로
갈 필요가 없다.

**왜 이렇게 도는가 (MCP 만으로는 안 된다).** MCP 는 클라이언트(Claude)가 서버를 부르는 한
방향이다. 서버가 Claude 에게 먼저 말을 걸려면 sampling(`sampling/createMessage`)이 필요한데
Claude Code 는 아직 지원하지 않는다 ([#1785](https://github.com/anthropics/claude-code/issues/1785),
2025-06 이후 열린 채). 그래서 방향을 뒤집었다:

```
입력창 → POST /coworker/ask → 서버가 `claude -p` 를 띄움
                                  └ 그 Claude 가 --attach 로 이 브리지에 얹혀 같은 탭을 만진다
                              ← SSE {chat:…} 로 진행·답변이 패널에 흐른다
```

- **`--attach` 붙이기 모드**가 핵심이다. 그냥 띄우면 두 번째 서버가 같은 포트를 잡으려다
  실패해(EADDRINUSE) 브리지 없는 껍데기가 된다. 붙이기 모드는 포트를 아예 잡지 않고
  도구 호출을 본체의 `/bridge/call` 로 넘긴다 — 활성 탭도 하나로 유지된다.
- **작업 디렉터리는 저장소 밖**(`%TEMP%/parti-coworker`)이다. `C:\Parti` 에서 띄우면 `.mcp.json`
  이 자동으로 읽혀 parti 서버가 한 벌 더 뜬다. 코워커는 코드가 아니라 도면을 만지는 사람이다.
- **대화는 이어진다.** 세션 id 를 기억해 `--resume` 으로 넘긴다 — 그래서 "방금 만든 그 방에
  문 달아줘" 가 통한다(탭을 새로고침해도).
- **이미지는 통째로 넘기지 않는다.** 화면에서 `PARTI_VISION` 이 먼저 읽고 *숫자와 확신도만*
  글로 붙여 보낸다 — Parti 의 최상위 원칙이다.
- **비용을 숨기지 않는다.** 한 마디마다 Claude 가 실제로 돌기 때문에, 끝날 때 그 요청의
  대략 비용을 패널에 적는다. `⏹ 중단` 으로 도는 도중에 끊을 수 있다.

`claude` 실행 파일은 PATH 에서 찾는다(npm 전역 설치본은 패키지 안의 `bin/claude.exe`).
못 찾으면 `PARTI_CLAUDE_BIN` 에 경로를 넣는다. ★셸을 거치지 않는다 — 사용자가 친 문장이
그대로 argv 로 들어가므로 셸을 끼우면 명령 주입이 된다.
...
그래서 루프백이 아닌 접속은 `/bridge/events` · `/bridge/result` · `/bridge/call` · `/coworker/*` 가
모두 토큰을 요구한다
